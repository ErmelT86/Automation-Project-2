...
describe('Issue create', () => {
  ...
  it('Test Case 2: Random Data Plugin Issue Creation', () => {
    const randTitle = faker.lorem.words(1);
    const randDescription = faker.lorem.sentences(3);
    cy.get('[data-testid="modal:issue-create"]').within(() => {
      cy.get('.ql-editor').type(randDescription);
      cy.get('.ql-editor').should('have.text', randDescription);
      cy.get('input[name="title"]').type(randTitle);
      cy.get('input[name="title"]').should('have.value', randTitle);
      cy.get('[data-testid="select:type"]').click();
